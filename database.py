...
def process_and_cache_features(db_file, config: Config):
    ...
    Args:
        ...
        more_scan (bool): Включает multi-crop извлечение признаков для каждого изображения.
        batch_size (int): Количество файлов для обработки перед сохранением в БД.
    """
    runtime.LOGGER.info(f"Проверка и обновление кэша признаков в файле: {db_file}")
    conn = create_connection(db_file)
    if conn is None:
        runtime.LOGGER.error(" ! Не удалось создать соединение с базой данных.")
        return
    ...
    try:
        ...
        if num_existing > 0:
            runtime.LOGGER.info(f"База данных найдена. В ней уже есть {num_existing} записей.")
        ...
        if not paths_to_process:
            runtime.LOGGER.info("База данных актуальна. Новых файлов для обработки не найдено.")
            return
        
        model, hook = load_model(config.model.model_name)

        runtime.LOGGER.info(f"Найдено {len(paths_to_process)} новых изображений для обработки.")
        ...
        if use_gpu and max_workers_global > 1:
            runtime.LOGGER.info("GPU inference detected; forcing feature extraction to run sequentially to avoid CUDA thread contention.")
        ...
        def _process_path(full_path):
            try:
                ...
                runtime.LOGGER.error(f" ! Error processing image {full_path}: {e}")
                return None
        ...
        for i in range(0, len(paths_to_process), config.model.batch_size):
            ...
            if batch_feats_data:
                try:
                    ...
                    runtime.LOGGER.info(f"Saved {written} records (processed {len(batch_feats_data)} images).")
                    existing_paths_in_db.update(canonical_path(item[0]) for item in batch_feats_data)
                except Exception as e:
                    runtime.LOGGER.error(f" ! Failed to write batch into database: {e}")
                    conn.rollback()
        del model
...
def load_features_from_db(db_file):
    ...
    runtime.LOGGER.info("Проверка актуальности записей в базе данных...")
    ...
    if deleted_count > 0:
        runtime.LOGGER.info(f"Пропущено {deleted_count} записей для удаленных файлов.")
...
def list_database_files(db_file):
    ...
    if not os.path.exists(db_file):
        runtime.LOGGER.error(f"Database file not found: {db_file}")
        return []
    ...
    try:
        ...
        runtime.LOGGER.error("The features table is missing in the database.")
        conn.close()
        return []
...
def move_database_entries(db_file, old_root, new_root):
    ...
    if not os.path.isdir(old_root_abs):
        runtime.LOGGER.error(f"Source directory does not exist: {old_root_abs}")
        conn.close()
        return 0, 0, 0
    ...
    for row_id, stored_path in rows:
        ...
        if not os.path.exists(current_abs):
            runtime.LOGGER.warning(f"File is missing on disk, skipping: {current_abs}")
            missing += 1
            continue
        ...
        try:
            ...
            runtime.LOGGER.warning(f"Failed to compute relative path for: {current_abs}")
            skipped += 1
            continue
        ...
        if os.path.exists(destination_abs):
            runtime.LOGGER.warning(f"Destination already exists, skipping move: {destination_abs}")
            skipped += 1
            continue
        ...
        try:
            ...
            runtime.LOGGER.error(f"Failed to move '{current_abs}' to '{destination_abs}': {exc}")
            skipped += 1
            continue
